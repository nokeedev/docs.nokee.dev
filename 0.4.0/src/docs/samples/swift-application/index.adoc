:jbake-version: 0.4.0
:toc:
:toclevels: 1
:toc-title: Contents
:icons: font
:idprefix:
:jbake-status: published
:encoding: utf-8
:lang: en-US
:sectanchors: true
:sectlinks: true
:linkattrs: true
:gradle-user-manual: https://docs.gradle.org/6.2.1/userguide
:gradle-language-reference: https://docs.gradle.org/6.2.1/dsl
:gradle-api-reference: https://docs.gradle.org/6.2.1/javadoc
:gradle-guides: https://guides.gradle.org/
:jbake-permalink: swift-application
:jbake-archivebasename: SwiftApplication
:includedir: .
= Swift Application
:jbake-summary: Application implemented in Swift.
:jbake-type: sample_chapter
:jbake-tags: sample, application, swift, native, gradle
:jbake-category: Native Development
:jbake-description: See how the Nokee Gradle plugins are used for building a native application implemented in Swift.
:plugin-language: Swift
:plugin-language-id: swift

NOTE: You can open this sample inside an IDE using the https://www.jetbrains.com/help/idea/gradle.html#gradle_import_project_start[IntelliJ native importer], https://projects.eclipse.org/projects/tools.buildship[Eclipse Buildship], or <<../../manual/developing-with-xcode-ide.adoc#, Nokee's Xcode IDE plugin>>

This sample shows how a native application implemented in {plugin-language} can be built with Gradle.
The application has no dependencies, and the build has a minimal configuration.

====
[.multi-language-sample]
=====
.build.gradle
[source,groovy]
----
include::groovy-dsl/build.gradle[]
----
=====
[.multi-language-sample]
=====
.build.gradle.kts
[source,kotlin]
----
include::kotlin-dsl/build.gradle.kts[]
----
=====
====

To build the application:

[listing.terminal]
----
$ ./gradlew assemble

BUILD SUCCESSFUL
2 actionable tasks: 2 executed
----

The application produced inside `./build/exes/main` directory:

[listing.terminal,only-if='macos']
----
$ ./build/exes/main/swift-application
Bonjour, Alice!
----

For more information, see <<../../manual/{plugin-language-id}-application-plugin.adoc#,{plugin-language} Application Plugin>> reference chapter and <<../../manual/building-native-projects.adoc#,Building Native Projects>> chapter.
